...
const STORAGE_KEY = "kyungja_v4";
const ROOM_KEY = "kyungja_room_v4";
const NAME_KEY = "kyungja_name_v4";
...
async function aiClassify(text, type) {
  var sys = "당신은 가계부 AI입니다. 사용자 입력에서 아래를 추출하세요.\n- type: "+type+"\n- category: "+(type==="수입"?"월급,알바,용돈,부업,기타수입":"식비,주거,교통,보험,의료,문화여가,의류,교육,기타,저축,투자")+" 중 하나\n- amount: 숫자만. 만→x10000, 천→x1000\n- description: 10자 이내\nJSON만 반환. {\"type\":\""+type+"\",\"category\":\"식비\",\"amount\":8000,\"description\":\"김치찌개\"}";
...
export default function App() {
  ...
  useEffect(function(){
    if(!roomCode) return;
    syncRoom();
    var id = setInterval(syncRoom, 10000);
    return function(){ clearInterval(id); };
  },[roomCode, myName]);

  useEffect(function(){
    if(!roomCode || !myName || myRecords.length === 0) return;
    uploadMyRecords();
  },[myRecords]);

  async function uploadMyRecords() {
    if(!roomCode || !myName) return;
    try {
      await fbSet("rooms/"+roomCode+"/members/"+myName, myRecords);
    } catch(e){}
  }
  ...
  async function handleIncome() {
    ...
    try {
      ...
      var now = new Date();
      var rec = {id:Date.now(),type:"수입",category:result.category,amount:result.amount,description:result.description,date:now.toLocaleDateString("ko-KR"),monthKey:getMonthKey(now),ts:now.getTime()};
      setMyRecords(function(prev){ return [rec].concat(prev); });
      setIncomeText("");
  ...
  async function handleExpense() {
    ...
    try {
      ...
      var now = new Date();
      var rec = {id:Date.now(),type:"지출",category:result.category,amount:result.amount,description:result.description,date:now.toLocaleDateString("ko-KR"),monthKey:getMonthKey(now),ts:now.getTime()};
      setMyRecords(function(prev){ return [rec].concat(prev); });
      setExpenseText("");
  ...
  async function handleCreateRoom() {
    ...
    var code = genCode();
    try {
      await fbSet("rooms/"+code+"/created", Date.now());
      await fbSet("rooms/"+code+"/members/"+inputName, myRecords.length>0?myRecords:["init"]);
      ...
      showToast("생성 실패. 다시 시도해주세요",false);
    }
  }
  ...
  async function handleJoinRoom() {
    ...
    try {
      ...
      showToast("참여 실패. 다시 시도해주세요",false);
    }
  }

  async function handleLeaveRoom() {
    if(roomCode && myName) {
      try { await fbSet("rooms/"+roomCode+"/members/"+myName, null); } catch(e){}
    }
    ...
    members.forEach(function(name){
      ...
      if(Array.isArray(recs)) {
        recs.forEach(function(r){
          if(r&&r.type) allRoomRecs.push(Object.assign({},r,{memberName:name}));
        });
      }
    });
  }
  ...
  myRecords.concat(allRoomRecs).forEach(function(r){
    if(r&&r.monthKey&&!seenM[r.monthKey]){ seenM[r.monthKey]=true; allMonths.push(r.monthKey); }
  });
  ...
  var memberStats = members.map(function(name,i){
    var recs = roomData&&roomData.members&&Array.isArray(roomData.members[name]) ? roomData.members[name].filter(function(r){ return r&&r.type&&r.monthKey===viewMonth; }) : [];
    ...
    return {name:name,income:inc,expense:exp,balance:inc-exp,color:MEMBER_COLORS[i%MEMBER_COLORS.length]};
  });
      ...
      <div style={S}>
        <div style={{background:"linear-gradient(160deg,#1E1B4B 0%,#0A0F1E 100%)",padding:"36px 24px 24px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18}}>
            <div>
              <div style={{fontSize:22,fontWeight:900,color:"#A5B4FC"}}>경자야 놀자</div>
              <div style={{fontSize:12,color:"#475569",marginTop:2}}>경제적 자유를 위한 기록</div>
            </div>
            <button onClick={function(){ setScreen("family"); }} style={{background:roomCode?"rgba(129,140,248,0.15)":"linear-gradient(90deg,#6366F1,#8B5CF6)",border:roomCode?"1px solid #6366F1":"none",color:"#fff",borderRadius:12,padding:"8px 14px",fontSize:13,fontWeight:700,cursor:"pointer"}}>
              {roomCode?"가족 "+members.length+"명":"+ 가족 연동"}
            </button>
          </div>
        ...
        <div style={{padding:"20px 24px 0"}}>
          <div style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <input ref={incRef} value={incomeText} onChange={function(e){ setIncomeText(e.target.value); }} onKeyDown={function(e){ if(e.key==="Enter"){ handleIncome(); }}} placeholder="예) 월급 300만원, 알바비 50만원" style={{flex:1,background:"#1E293B",border:"2px solid #064E3B",color:"#F1F5F9",borderRadius:14,padding:"16px 18px",fontSize:16,outline:"none",fontFamily:"inherit"}} />
          ...
          <div style={{marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <input ref={expRef} value={expenseText} onChange={function(e){ setExpenseText(e.target.value); }} onKeyDown={function(e){ if(e.key==="Enter"){ handleExpense(); }}} placeholder="예) 점심 8천원, 마트 45000원" style={{flex:1,background:"#1E293B",border:"2px solid #7F1D1D",color:"#F1F5F9",borderRadius:14,padding:"16px 18px",fontSize:16,outline:"none",fontFamily:"inherit"}} />
          ...
          {todayRecs.length>0&&(
            <div>
              <div style={{fontSize:12,color:"#475569",marginBottom:8,fontWeight:600}}>오늘 기록</div>
      ...
      <div style={S}>
        <div style={{padding:"28px 24px 20px",borderBottom:"1px solid #1E293B"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            ...
            <select value={viewMonth} onChange={function(e){ setViewMonth(e.target.value); }} style={{background:"#1E293B",border:"1px solid #334155",color:"#CBD5E1",borderRadius:10,padding:"8px 12px",fontSize:14,cursor:"pointer"}}>
        ...
        <div style={{padding:"20px 24px"}}>
          {monthRecs.length===0?(
            <div style={{textAlign:"center",padding:"60px 0",color:"#334155"}}>
              ...
              <div style={{fontSize:18,fontWeight:700}}>기록이 없어요</div>
            </div>
          ):(
  ...
  function renderReport() {
    var rRecs=myRecords.filter(function(r){ return r&&r.monthKey===viewMonth; });
    var rInc=rRecs.filter(function(r){ return r.type==="수입"; }).reduce(function(s,r){ return s+r.amount; },0);
    var rExp=rRecs.filter(function(r){ return r.type==="지출"; }).reduce(function(s,r){ return s+r.amount; },0);
    var rBal=rInc-rExp;
    var rSave=rInc>0?Math.round((rBal/rInc)*100):0;
    var rByCat={};
    rRecs.filter(function(r){ return r.type==="지출"; }).forEach(function(r){ rByCat[r.category]=(rByCat[r.category]||0)+r.amount; });
    var health=Math.max(0,100-(Object.keys(rByCat).filter(function(k){ var cat=CATS.find(function(c){ return c.key===k; }); return cat&&rInc>0&&Math.round((rByCat[k]/rInc)*100)>cat.pct; }).length*12)-(rSave<15?20:0));
    return (
      <div style={S}>
        <div style={{padding:"28px 24px 20px",borderBottom:"1px solid #1E293B"}}>
          <button onClick={function(){ setScreen("home"); }} style={{background:"none",border:"none",color:"#64748B",fontSize:15,cursor:"pointer",marginBottom:16,padding:0}}>← 뒤로</button>
        ...
        <div style={{padding:"24px"}}>
          <div style={{background:"linear-gradient(135deg,#1E1B4B,#4C1D95)",borderRadius:20,padding:"24px",marginBottom:22}}>
            ...
            <div style={{display:"flex",gap:10,marginBottom:16}}>
              ...
              <div style={{flex:1,background:"rgba(255,255,255,0.1)",borderRadius:14,padding:"12px 16px"}}>
                ...
                <div style={{fontSize:30,fontWeight:900,color:rSave>=15?"#86EFAC":rSave>=0?"#FDE68A":"#FCA5A5"}}>{rSave}<span style={{fontSize:13,color:"#A5B4FC"}}>%</span></div>
              </div>
            </div>
            <div style={{display:"flex",gap:8}}>
              {[["수입",rInc,"#22C55E"],["지출",rExp,"#F87171"],["잔액",rBal,rBal>=0?"#A5F3FC":"#FCA5A5"]].map(function(x){
          ...
          <div style={{background:"#1E293B",borderRadius:16,overflow:"hidden",marginBottom:20}}>
            {CATS.filter(function(c){ return rByCat[c.key]>0; }).length===0?
              <div style={{padding:"20px",textAlign:"center",color:"#475569",fontSize:13}}>지출 기록이 없어요</div>
            :CATS.filter(function(c){ return rByCat[c.key]>0; }).map(function(cat,i,arr){
              var spent=rByCat[cat.key]||0;
              var rec=Math.round(rInc*cat.pct/100);
              var over=rInc>0&&spent>rec;
              return (
            ...
            <div style={{background:"#1E293B",borderRadius:14,padding:"16px 18px"}}>
              ...
              {CATS.map(function(cat){
                return <div key={cat.key} style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:7}}>
                  ...
                  <span style={{color:cat.color,fontWeight:600}}>{fmt(Math.round(rInc*cat.pct/100))+" ("+cat.pct+"%)"}</span>
                </div>;
              })}
            </div>
          )}
        ...
        <div style={{padding:"28px 24px"}}>
          ...
          {!roomCode?(
            <div>
              ...
              <input value={inputName} onChange={function(e){ setInputName(e.target.value); }} placeholder="예) 아빠, 엄마, 홍길동" style={{width:"100%",background:"#1E293B",border:"1px solid #334155",color:"#F1F5F9",borderRadius:12,padding:"14px 16px",fontSize:16,outline:"none",boxSizing:"border-box",marginBottom:12}} />
              ...
              <div style={{background:"#1E293B",borderRadius:16,padding:"20px"}}>
                ...
                <input value={inputCode} onChange={function(e){ setInputCode(e.target.value.toUpperCase()); }} placeholder="6자리 코드" maxLength={6} style={{width:"100%",background:"#0A0F1E",border:"1px solid #334155",color:"#F1F5F9",borderRadius:12,padding:"14px",fontSize:22,fontWeight:800,letterSpacing:6,outline:"none",boxSizing:"border-box",marginBottom:10,textAlign:"center"}} />
  ...
  if(screen==="familyDone") return renderFamilyDone();
  return renderHome();
}